...
describe('adminQuizDescriptionUpdate', () => {
  ...
  describe('Testing for correct return type', () => {
    test('Update quiz description successfully', () => {
      expect(adminQuizDescriptionUpdate(user.authUserId, quizId, 
      'New quiz description')).toStrictEqual({});
    });
  });
  ...
  describe('Testing authUserId in adminQuizDescriptionUpdate', () => {
    test('Invalid authUserId', () => {
      expect(adminQuizDescriptionUpdate('invalidUser123', quizId, 
      'New Description')).toStrictEqual(error);
    });
  });

  describe('Testing quizId in adminQuizDescriptionUpdate', () => {
    test('Invalid quizId', () => {
      expect(adminQuizDescriptionUpdate(user.authUserId, 'invalidQuiz123', 
      'New Description')).toStrictEqual(error);
    });
    test('Quiz not owned by user', () => {
      ...
      'Description');
      expect(adminQuizDescriptionUpdate(user.authUserId, otherQuiz.quizId, 
      'New Description')).toStrictEqual(error);
    });
  });
  ...
  describe('Testing description in adminQuizDescriptionUpdate', () => {
    test('Description is more than 100 characters', () => {
      const longDescription = 'Okay deep breath. This is going to be a really \
      really really long quiz description. Like really long. Absurdly inconviently long.';
